...
def main():
    ...
    clip_D = [p.assign(tf2.clip_by_value(p, -0.01, 0.01)) for p in d_vars]
    ...
    capped_gvs = [(tf2.clip_by_value(grad, -.25, .25), var) for grad, var in gvs]
    ...
    while global_step < args.num_steps:
        for _ in range(min(total_steps,args.num_steps-global_step)):
            ...
            if global_step % 999 == 0:
                ...
                remaining_seconds = estimated_time % (24 * 3600)
                ...
                # Calculate ETA as the current time + estimated time
                eta = current_time + datetime.timedelta(seconds=estimated_time)
